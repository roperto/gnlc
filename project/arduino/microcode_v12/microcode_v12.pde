...
#define TIME_MINIMUM_BETWEEN_LAPS           100e3 // 0.1s, software must use higher number based on length of track
...
#define A2P_RESERVED_58                   (byte)0x58 // 01011.---
  ...
  if (comm_next <= us) {
    ...
    while (Serial.available() > 0) {
      ...
      case P2A_REQUEST_SET_ALL_RELAYS: //set all relays to param
        ...
        //set lights
        digitalWrite(pins_light[0], !comm_param);
        digitalWrite(pins_light[1], !comm_param);
        //finish
        break;
      ...
      case P2A_REQUEST_RELAY_PAUSE: //pause/restore relays
        if (comm_param) { //restore
          digitalWrite(pins_light[0], 1);
          digitalWrite(pins_light[1], 1);
          for (t=0; t<MAX_LANES; t++) {
            digitalWrite(pins_relays[t], sensor_pause[t]);
          }
        }
        else { //pause
          digitalWrite(pins_light[0], 0);
          digitalWrite(pins_light[1], 0);
          for (t=0; t<MAX_LANES; t++) {
